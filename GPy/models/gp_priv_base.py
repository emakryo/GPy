...
class GPPrivBase(Model):
    def __init__(self, X, Y, Xstar, kernel=None, kernel_star=None,
        ...
        if np.isscalar(mean):
            self.mean = Constant(X.shape[1], 1, mean)
        else:
            self.mean = mean

        if np.isscalar(mean_star):
            self.mean_star = Constant(Xstar.shape[1], 1, mean_star)
        else:
    ...
    def _ep(self, K, Kstar, Y, mean, mean_star, diag_bias=1e-5):
        ...
        for loop in range(self.max_iter):
            ...
            if not self.parallel_update:
                ...
                nu, tau, nu_star, tau_star, log_Z = zip(*pool.map(
                    self.__class__._next_site_pool, args))
                # _next_sites = []
    ...
    @classmethod
    def _next_site_pool(cls, args):
        return cls._next_site(args[0], args[1], args[2])
...
class SiteParam:
    def __init__(self, n, epsilon=1e-3, rtol=1e-5, atol=1e-8):
        self.n = n
        self.nu = np.zeros(n)
        self.tau = np.zeros(n)
        self.rtol = rtol
    ...
    def is_close(self, other):
        ...
        return max(np.max(np.abs(self.nu-other.nu)),
                   np.max(np.abs(self.tau-other.tau))) < self.epsilon
